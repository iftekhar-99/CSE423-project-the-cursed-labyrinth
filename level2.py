...
boss_obj = {
    'x': 0, 'z': 0, 
    'hp': 5000,          # Increased HP for better fight
    'active': False, 
    'dead': False, 
    'shoot_timer': 0     # NEW: Timer for boss attacks
}
...
def generate_level_2():
    global map_data, player_pos, display_list, birds, boss_obj, pistons
    ...
    # --- A. MAZE GENERATION (Wide Paths) ---
    
    # 1. Force Clear Start Area
    for i in range(start_x-2, start_x+3):
        for j in range(start_y-2, start_y+3):
             if 0 <= i < MAP_DIM and 0 <= j < MAP_DIM: map_data[i][j] = C_EMPTY

    # 2. Main Highway to Boss
    cx, cy = start_x, start_y
    while (abs(cx - boss_x) > 1 or abs(cy - boss_y) > 1):
        if 0 <= cx < MAP_DIM and 0 <= cy < MAP_DIM: map_data[cx][cy] = C_EMPTY
        if 0 <= cx+1 < MAP_DIM and 0 <= cy < MAP_DIM: map_data[cx+1][cy] = C_EMPTY
        if 0 <= cx < MAP_DIM and 0 <= cy+1 < MAP_DIM: map_data[cx][cy+1] = C_EMPTY
    ...
    diggers = []
    for _ in range(30):
        diggers.append({'x': random.randint(5, MAP_DIM-5), 'y': random.randint(5, MAP_DIM-5), 'life': 200})
    ...
    for x in range(MAP_DIM):
        for z in range(MAP_DIM):
            if map_data[x][z] == C_EMPTY:
                ...
                # 2. SPIKES (INCREASED to 7%)
                elif r < 0.07:  # <--- MORE SPIKES!
                ...
                # 3. SHOOTING BIRDS (5%)
                    ...
                    birds.append({
                        'x': wx, 'y': 35, 'z': wz, 
                        'angle': 0, 'speed': 1.5,
                        'shoot_timer': random.randint(0, 100) # NEW: Reload timer
                    })
    ...
    for x in range(MAP_DIM):
        for z in range(MAP_DIM):
            ...
            if cell == C_WALL:
                ...
                glColor3f(0.5, 0.2, 0.1); glutSolidCube(CELL_SIZE)
...
def draw_entities():
    ...
    for p in pistons:
        ...
        glColor3f(0.2, 0.2, 0.2); glScalef(1, 2, 1); glutSolidCube(18)
        glColor3f(0.1, 0.1, 0.1); glTranslatef(0, 10, 0); glScalef(0.2, 5, 0.2); glutSolidCube(18)
        glPopMatrix()
    ...
    for b in birds:
        ...
        glColor3f(0.9, 0.2, 0.2) # Reddish aggressive birds
    ...
    # --- 8. NEW BOSS MODEL (Square + Rounded Objects) ---
    if not boss_obj['dead']:
        glPushMatrix()
        glTranslatef(boss_obj['x'], 30, boss_obj['z'])
        
        # 1. Main Body (Large Red Cube)
        glPushMatrix()
        glColor3f(0.8, 0, 0)
        glScalef(2, 2, 2)
        glutSolidCube(15)
        ...
        # 2. Core (Sphere inside)
        glPushMatrix()
        glColor3f(0.2, 0, 0) # Dark red core
        glutSolidSphere(12, 10, 10)
        glPopMatrix()

        # 3. Head (Rounded Object on Top)
        glPushMatrix()
        glTranslatef(0, 20, 0)
        glColor3f(1.0, 0.2, 0.2) # Bright red
        glutSolidSphere(10, 15, 15)
        # Eye (Cube inside sphere)
        glTranslatef(0, 0, 8)
        glColor3f(1, 1, 0) # Yellow Eye
        glutSolidCube(6)
        glPopMatrix()

        # 4. Floating Rotating Shield (Square Objects)
        glPushMatrix()
        glRotatef(global_time * 60, 0, 1, 0) # Spin fast
        for i in range(4):
            glPushMatrix()
            glRotatef(i * 90, 0, 1, 0)
            glTranslatef(35, 0, 0) # Distance from body
            
            # Shield Element: Cube with Sphere ends
            glColor3f(0.4, 0.4, 0.4)
            glutSolidCube(8)
            
            glPushMatrix(); glTranslatef(0, 5, 0); glColor3f(1,0,0); glutSolidSphere(3,8,8); glPopMatrix()
            glPushMatrix(); glTranslatef(0,-5, 0); glColor3f(1,0,0); glutSolidSphere(3,8,8); glPopMatrix()
...
def draw_hud():
    ...
    # Boss HP Bar (Only when close)
    dist_to_boss = math.sqrt((player_pos[0]-boss_obj['x'])**2 + (player_pos[2]-boss_obj['z'])**2)
    if dist_to_boss < 500 and not boss_obj['dead']:
        glColor3f(0.5, 0, 0)
        glRectf(WINDOW_WIDTH/2 - 200, WINDOW_HEIGHT - 60, WINDOW_WIDTH/2 + 200, WINDOW_HEIGHT - 40)
        glColor3f(1, 0, 0)
        width = (boss_obj['hp'] / 5000) * 400
        glRectf(WINDOW_WIDTH/2 - 200, WINDOW_HEIGHT - 60, WINDOW_WIDTH/2 - 200 + width, WINDOW_HEIGHT - 40)
        glRasterPos2f(WINDOW_WIDTH/2 - 40, WINDOW_HEIGHT - 55)
        glutBitmapString(GLUT_BITMAP_HELVETICA_18, b"BOSS")
    ...
    if emp_cooldown <= 0:
        glColor3f(1, 1, 0); glRasterPos2f(20, WINDOW_HEIGHT - 70); glutBitmapString(GLUT_BITMAP_HELVETICA_18, b"EMP READY [E]")
    else:
        glColor3f(0.5, 0.5, 0.5); glRasterPos2f(20, WINDOW_HEIGHT - 70); glutBitmapString(GLUT_BITMAP_HELVETICA_18, b"EMP RECHARGING...")
...
def update_physics():
    ...
    global projectiles, birds, player_daggers, loot, particles, pistons, boss_obj
    global spike_cooldown, game_over, level_complete, emp_cooldown, immunity_timer

    # 1. RESTART CHECK (First Priority)
    ...
    if key_states.get(b' ') and player_stamina > 0:
        current_speed = base_speed * 2.0; player_stamina -= 1.0 
    else:
        if player_stamina < 100: player_stamina += 0.3 
    ...
    if key_states.get(b'e') and emp_cooldown <= 0:
        ...
        projectiles = [] 
        for _ in range(30):
            particles.append({'x': player_pos[0], 'y': 20, 'z': player_pos[2], 'dx': random.uniform(-2,2), 'dy': random.uniform(-2,2), 'dz': random.uniform(-2,2), 'life': 40})
    ...
    # --- B. OBSTACLE & ENEMY ATTACKS ---
    gx = int(round(player_pos[0] / CELL_SIZE)); gz = int(round(player_pos[2] / CELL_SIZE))
    ...
    for b in birds:
        ...
        # NEW: Bird Shooting Logic
        b['shoot_timer'] += 1
        dist_to_player = math.sqrt((b['x']-player_pos[0])**2 + (b['z']-player_pos[2])**2)
        if b['shoot_timer'] > 100 and dist_to_player < 300: # Fire every 100 frames if close
            b['shoot_timer'] = 0
            # Calculate angle to player
            aim_angle = math.atan2(player_pos[0] - b['x'], player_pos[2] - b['z'])
            projectiles.append({
                'x': b['x'], 'z': b['z'], 
                'dx': math.sin(aim_angle)*2.5, # Fast bird bullets
                'dz': math.cos(aim_angle)*2.5, 
                'life': 100
            })

    # NEW: BOSS LOGIC
    if not boss_obj['dead']:
        boss_dist = math.sqrt((boss_obj['x']-player_pos[0])**2 + (boss_obj['z']-player_pos[2])**2)
        if boss_dist < 600: # Only attack if player is in the arena
            boss_obj['shoot_timer'] += 1
            
            # ATTACK: SHOCKWAVE (Every 60 frames / 1 sec approx)
            if boss_obj['shoot_timer'] > 60:
                boss_obj['shoot_timer'] = 0
                # Fire 8 bullets in a ring
                for i in range(8):
                    angle = (i * 45) * (math.pi / 180)
                    projectiles.append({
                        'x': boss_obj['x'], 'z': boss_obj['z'],
                        'dx': math.sin(angle) * 3.0,
                        'dz': math.cos(angle) * 3.0,
                        'life': 150
                    })
    ...
    for d in player_daggers:
        ...
        if not hit and not boss_obj['dead']:
            if math.sqrt((d['x']-boss_obj['x'])**2 + (d['z']-boss_obj['z'])**2) < 30:
                boss_obj['hp'] -= 50; hit = True # Daggers deal 50 dmg
        ...
        if 0 <= dgx < MAP_DIM and 0 <= dgz < MAP_DIM:
            if map_data[dgx][dgz] == C_SPIKE:
                map_data[dgx][dgz] = C_EMPTY; hit = True; loot.append({'x':d['x'], 'z':d['z'], 'type': 'HP' if random.random() < 0.8 else 'AMMO'})
...
def main():
    ...
    glutCreateWindow(b"Level 2: The Iron Gauntlet - Boss Fight")
    glEnable(GL_DEPTH_TEST)
