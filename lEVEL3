...
# Scale Factors (Matched to Level 1)
MAP_DIM = 140        
CELL_SIZE = 30       
WALL_HEIGHT = 70    
...
# --- ENTITY LISTS ---
mines = []          # Landmines (Preserved)
particles = [] 
animals = []        # Rabbits (Preserved)
animal_rocks = []   # Projectiles (From Rabbits)
daggers = []        # Player projectiles
loot = []           # HP Balls, Ammo, and SHIELDS
boulders = []       # Boss Rain & Gas Cloud Rain
boss_minions = []   # Purple Enemies 
gas_clouds = []     # Green Spheres (Now drop boulders)
...
# --- PLAYER STATE ---
player_pos = [0, 0, 0]
player_angle = 1.57   # Left/Right
player_pitch = 0.0    # Up/Down
base_speed = 3.5
...
is_exhausted = False
is_sprinting = False
shield_timer = 0      # Existing Shield Timer

# --- NEW FEATURES FROM LEVEL 1 ---
inventory = []
immunity_timer = 0
infinite_stamina_timer = 0
# Map toggle removed (Map is now always visible)
...
def generate_level_3():
    ...
    # Connect Rooms
    boss_room['center'] = (MAP_DIM - 35, MAP_DIM - 20) 
    ...
    for x in range(MAP_DIM):
        for z in range(MAP_DIM):
            ...
            if cell == C_WALL:
                ...
                if (x+z)%2 == 0: glColor3f(0.2, 0.15, 0.1) 
                else: glColor3f(0.25, 0.2, 0.15) 
                glBegin(GL_QUADS)
...
def draw_entities():
    ...
    for p in particles:
        ...
        # Blue particles if shield was hit, Red/Orange for others
    ...
    for l in loot:
        ...
        elif l.get('type') == 'AMMO':
            # CYAN DAGGER BUNDLE (Ammo)
            glColor3f(0.0, 1.0, 1.0)
    ...
    if boss_active and not boss_defeated:
        ...
        glScalef(4, 4, 4) 
        glColor3f(0.4, 0.3, 0.2); glutSolidCube(10) # Body
        glTranslatef(0, 8, 0); glColor3f(0.5, 0.4, 0.3); glutSolidSphere(4, 10, 10) # Head
        glColor3f(1, 0, 0) # Eyes
...
def draw_hud():
    ...
    # SHIELD & IMMUNITY STATUS
    status_y = WINDOW_HEIGHT - 90
    if shield_timer > 0:
        glColor3f(0, 0.5, 1)
        glRasterPos2f(20, status_y)
        s_msg = f"SHIELD ACTIVE: {int(shield_timer/60)}s"
...
def update_physics():
    ...
    # --- MOVEMENT (Strict Stop Logic) ---
    move_speed = 0
    strafe = 0
    
    # Only assign speed if key is actively held.
    # This ensures that if keys are not pressed (or keyUp has triggered), 
    # speed remains 0, preventing "automated running".
    ...
    # Clamp Pitch (-85 to +85 degrees approx)
    ...
    for m in mines:
        ...
        if m['state'] == 'ACTIVE':
            ...
            if m['timer'] <= 0:
                ...
                if d_player < 40 and immunity_timer <= 0: player_hp -= 50
        active_mines.append(m)
    mines = active_mines
    
    # --- GAS CLOUDS (Drop Boulders) ---
    for g in gas_clouds:
        d = dist(player_pos, (g['x'], 0, g['z']))
        if d < 400: # If player is in range
            g['timer'] += 1
            if g['timer'] > 400: # Wait approx 6-7 seconds then rain
                # DROP BOULDER RAIN (High Y)
                drop_x = player_pos[0] + random.randint(-40, 40)
                drop_z = player_pos[2] + random.randint(-40, 40)
                boulders.append({'x': drop_x, 'y': 250, 'z': drop_z, 'vy': 0})
                g['timer'] = 0
    ...
    for a in animals:
        ...
        if dist_p < 200: 
            ...
            if 0 <= agx < MAP_DIM and 0 <= agz < MAP_DIM and map_data[agx][agz] != C_WALL:
                ...
                if 0 <= agx < MAP_DIM and map_data[agx][int(a['z']//CELL_SIZE)] != C_WALL: a['x'] = nx
                elif 0 <= agz < MAP_DIM and map_data[int(a['x']//CELL_SIZE)][agz] != C_WALL: a['z'] = nz
    ...
    # --- PURPLE ENEMIES (Chase) ---
    enemy_speed = base_speed / 8.0 
    for m in boss_minions:
        dx = player_pos[0] - m['x']; dz = player_pos[2] - m['z']
        dst = math.sqrt(dx*dx + dz*dz)
        if dst < 15 and immunity_timer <= 0: player_hp -= 0.5
        elif dst < 400:
             m['x'] += (dx/dst) * enemy_speed
             m['z'] += (dz/dst) * enemy_speed

    # --- BOSS LOGIC (Tank Golem) ---
    if boss_active:
        boss_obj['timer'] += 1
        ...
        # 1. SMACK ATTACK (Proximity)
        if d_to_p < 30:
            if immunity_timer <= 0: player_hp -= 1.0 # Rapid damage
            # Knockback
        ...
        # 3. ATTACK: BOULDER RAIN (Constant)
        if boss_obj['timer'] % 60 == 0:
            boulders.append({'x': player_pos[0], 'y': 250, 'z': player_pos[2], 'vy': 0})
            
        # 4. ABILITY: SPAWN MINIONS
        ...
        if boss_obj['timer'] % 300 == 0:
            if boss_obj['hp'] < boss_obj['max_hp']:
                boss_obj['hp'] += 20
                for _ in range(5): particles.append({'x': bx, 'y': 30, 'z': bz, 'dx': 0, 'dy': 2, 'dz': 0, 'life': 20})

    # --- BOULDERS (Physics & Shielding) ---
    active_boulders = []
    for b in boulders:
        b['vy'] += 0.5; b['y'] -= b['vy']
        if b['y'] <= 0:
            ...
            if dist(player_pos, (b['x'], 0, b['z'])) < 30: 
                if shield_timer > 0:
                     # Shield blocked it! Blue particles
        ...
        if not hit:
            ...
            for a in animals:
                if dist((d['x'], 0, d['z']), (a['x'], 0, a['z'])) < 15:
                    ...
                    loot.append({'x': a['x'], 'z': a['z'], 'type': l_type})
                else: active_animals.append(a)
            animals = active_animals

        # Hit Minions -> Drop random loot
        if not hit:
            active_minions = []
            for m in boss_minions:
                if dist((d['x'], 0, d['z']), (m['x'], 0, m['z'])) < 15:
                    ...
                    loot.append({'x': m['x'], 'z': m['z'], 'type': l_type})
    ...
    for l in loot:
        if dist(player_pos, (l['x'], 0, l['z'])) < 15: 
            if l.get('type') == 'HP':
                if player_hp >= 100: inventory.append("HP Pill") # Inventory logic
    ...
    for r in animal_rocks:
        ...
        if dist(player_pos, (r['x'], 0, r['z'])) < 10:
            if immunity_timer <= 0: player_hp -= 5
        ...
        if 0 <= rgx < MAP_DIM and 0 <= rgz < MAP_DIM and map_data[rgx][rgz] != C_WALL and r['life'] > 0: active_rocks.append(r)
...
def setup_camera():
    ...
    # Calculate Look Vector with Pitch
    lx = math.sin(player_angle) * math.cos(player_pitch)
    ly = math.sin(player_pitch)
    lz = -math.cos(player_angle) * math.cos(player_pitch)
    
    gluLookAt(player_pos[0], 25, player_pos[2], 
              player_pos[0] + lx, 25 + ly, player_pos[2] + lz, 
              0, 1, 0)
...
def keyDown(key, x, y):
    ...
    # Level 1 Features
    # Note: 'c' toggle for map removed as requested (map is now always on)
    
    if key == b'1' and "Immunity" in inventory: 
        inventory.remove("Immunity")
        immunity_timer = 360
    if key == b'2' and "HP Pill" in inventory and player_hp < 100: 
    ...
    if button == GLUT_LEFT_BUTTON and state == GLUT_DOWN and not game_over:
        if player_ammo > 0:
            ...
            daggers.append({
                ...
                'angle': player_angle, 'pitch': player_pitch, 'life': 60
            })
...
def showScreen():
    ...
    draw_entities()
    draw_minimap() # Minimap is always visible now
    draw_hud()
    glutSwapBuffers()
...
def init_fog():
    # Adjusted fog color to match Stone level (Greyish)
    glEnable(GL_FOG); glFogfv(GL_FOG_COLOR, (0.1, 0.1, 0.1, 1.0))
    glFogi(GL_FOG_MODE, GL_LINEAR); glFogf(GL_FOG_START, 100.0); glFogf(GL_FOG_END, 800.0)
...
def main():
    ...
    glutCreateWindow(b"Level 3: The Final Stand")
    glEnable(GL_DEPTH_TEST)
    init_fog() # New Feature
    generate_level_3()
